...
//****************************For·搜索************************************
//for search·step 1
exports.getDataDic=function(str){
	var dic=new Array();
	for(var l=1;l<str.length+1;l++){
		var data={};
		for(var i=0;i<str.length-l+1;i++){
			data[str.substr(i,l)]=1;
		}
		dic.push(data);
		//cout(dic[l-1]);
	}
...
//for search·step 2
exports.getSimilarity=function(dic,all){
	var sum=0;
	var lsKey=new Array();
	var max=(all.length>dic.length)?dic.length:all.length;
	for(var l=1;l<max+1;l++){
		for(var i=0;i<all.length-l+1;i++){
			var tp=all.substr(i,l);
			if(dic[l-1][tp]){
				sum += l;
				lsKey.push(tp);
			}
	...
	return { value:sum,keys:lsKey };
}
//search
exports.getSearchByList=function(str,list){
	var rst=new Array();
	var dic=this.getDataDic(str);
	var lsv=new Array();
	var lsi=new Array();
	var lsKeys=new Array();
	for(var i=0;i<list.length;i++){
		var smt=this.getSimilarity(dic,list[i]);
		lsv.push(smt.value);
		lsKeys.push(smt.keys);
		lsi.push(i);
	}
	//排序
	for(var i=0;i<lsv.length;i++){
		for(var j=i+1;j<lsv.length;j++){
			if(lsv[i]<lsv[j]){
				var tpv=lsv[i];
				var tpi=lsi[i];
				lsv[i]=lsv[j];
	...
	for(var i=0;i<lsv.length;i++){
		if(lsv[i]==0)break;
		//索引，关键词列表
		rst.push({ index: lsi[i], keys:lsKeys[lsi[i]] });
	}
	return rst;
}
//****************************End·搜索************************************
